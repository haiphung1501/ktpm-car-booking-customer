...
MapScreen = ({route, navigation}) => {
  ...
  const [socket, setSocket] = useState(io("http://gofast-api.onrender.com/notification"));
  ...
  const YourLocationViewIc = () =>{
    return (<Image style={{width: 25, height: 25, marginLeft: -12}} source={require('../../assets/images/yourLocationIc.png')}/>)
  };

  const DestinationViewIc = () =>{
    return (<Image style={{width: 25, height: 25 , marginLeft: -12}} source={require('../../assets/images/destinationIc.png')}/>)
  };
    ...
    <SafeAreaView className="flex-1 bg-green-100">
        <View style = {styles.container}>
            <MapView
                ...
                <Marker coordinate={origin} draggable><YourLocationView/></Marker>
                <Marker coordinate={destination} draggable><DestinationView/></Marker>
                <MapViewDirections
                    origin={origin}
            ...
            <View style={{display: 'flex', justifyContent: 'space-between', height: '100%'}}>
                ...
                <TouchableOpacity 
                    ...
                    className = 'flex flex-row w-3/4 mb-2 bg-slate-100 p-2 h-10 rounded-md shadow-xl shadow-gray-600'>
                    <DestinationViewIc/>
                    <Text className = 'text-base'>{destination.address.substring(0, 35)}</Text>
                </TouchableOpacity>
                </View>
                ...
                <Animated.View style={[styles.button, { opacity: buttonOpacity }]}>
                    <View className="bg-white border border-slate-400 rounded-t-xl pt-4 w-full">
                      <TouchableOpacity
                          ...
                          style={{backgroundColor: selectedVehicle === 'goBike' ? '#BBF7D0' : 'white', width: '100%', padding: 5}}
                        >
                          <View className="flex flex-row justify-between items-center mb-2">
                            <View className="flex flex-row items-center">
                            <Image style={{width: 40, height: 40}} source={require('../../assets/images/motoImage.png')}/> 
                              <View className="ml-6">
                                <Text className= "font-bold text-black text-base">GoBike</Text>
                            ...
                            <Text className= "font-bold text-black text-base">15000 VND</Text>
                          </View>
                        </TouchableOpacity>
                        <TouchableOpacity
                          onPress={() => setSelectedVehicle('goCar')}
                          style={{backgroundColor: selectedVehicle === 'goCar' ? '#BBF7D0' : 'white', width: '100%', padding: 5}}>
                        <View className="flex flex-row justify-between items-center mb-2">
                          <View className="flex flex-row items-center">
                          <Image style={{width: 40, height: 40}} source={require('../../assets/images/carImage.png')}/> 
                            <View className="ml-6">
                              <Text className= "font-bold text-black text-base">GoCar</Text>
                              <View className="flex flex-row ">
                                <Text className="text-gray-500">3 phút - </Text>
                                <MaterialIcons name="person" size={18} color="gray"/> 
                          ...
                          <Text className= "font-bold text-black text-base">21000 VND</Text>
                        </View>
                        </TouchableOpacity>
                        <View className='flex items-center align-middle justify-center'>
                          ...
                          ><Text className = 'text-white font-bold text-base'>Đặt xe</Text></Button>
                        </View>
                  </View>
            ...
            <Animated.View style={[styles.bottomSheet, bottomSheetAnimation]}>
                <View className="mt-3 ml-6 flex flex-row items-center">
                  <View className="bg-white h-6 w-6 rounded-full items-center justify-center">
                    ...
                    <Text className="font-bold text-sm text-white ml-2">Bạn sẽ đến nơi trong </Text>
                  <View className="bg-green-500 rounded-xl ml-2 p-1">
                    <Text className="font-bold text-sm text-white "> {travelTime} phút </Text>
                  </View>
                </View>
                <View className="bg-white mt-3 h-full">
                    ...
                    <View className="flex flex-row-reverse px-5 items-center mb-1 justify-between">
                        <View className="w-14 h-14 rounded-full bg-gray-100">
                            <Image style={{width: 56, height: 56}} source={{uri: booking.userId.avatar.url}} />
                        </View>
                        <View className='flex flex-col'>
                            <Text className='font-bold text-black text-xl'>{booking.driverId.displayName}</Text>
                            <View className='flex flex-row mt-2'>
                                <MaterialIcons name="star" size={24} color="#FF8C00" />
                                <Text className='text-base text-gray-500'>{booking.driverId.driverRating}</Text>
                            </View>
                        </View>
                    </View>
                    <View className="p-3">
                        ...
                        <View className="bg-white border border-slate-400 rounded-lg p-3 mt-4">
                            <View className="relative">
                                ...
                                <Text className="font-medium text-gray-500 ml-9 mt-4">{(distance/1000).toFixed(1)} km - {travelTime} phút</Text>
                        ...
                        <TouchableOpacity className='flex items-center align-middle justify-center'>
                          <Button
                              className = 'mt-6 mb-1 p-1 rounded-lg bg-slate-400 w-3/5 h-14'
                          ><Text className = 'text-white font-bold text-lg'>Hủy chuyến</Text></Button>
                        </TouchableOpacity>
                    </View>
                </View>
            </Animated.View>)}
